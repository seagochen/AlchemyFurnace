import torch
from YoloVer1.grids.GeneralGrids import GeneralGrids
from YoloVer1.grids import BoundingBox as bbox


class YoloGrids(GeneralGrids):

    def __init__(self, grids_size, confidences, bounding_boxes, object_categories,
                 spatial_size: tuple = (448, 448), alpha: float = 0., beta: float = 448., gamma: float = 1.):
    ...
    def __call__(self, target, bounding_box) -> torch.Tensor:
        """
        根据用户输入的target，以及 bounding box 坐标（左上x，左上y，右下x，右下y）生成对应的yolo网格信息
        :param target:
        :param bounding_box:
        :return:
        """
        # calculate the bounding box relative coordination
        general_coord = bbox.grids_coord(bounding_box,
        ...
        return self.tensor.reshape(-1, self.grids_size[0] * self.grids_size[1])
    ...
    #     return self.tensor
    #
    # def get_ltrb_coord(self, grid_i, grid_j, bbox_ind, gamma=448) -> tuple:
    #     # focus on grid
    #     grid = self.focus_cursor(grid_i, grid_j)
    #
